...
/**
 * Calculates the user's trust badge tier based on their global reputation.
 * @param {number} reputation - The user's global reputation.
 * @returns {object} Badge tier details containing label, color, and icon.
 */
export const getTrustBadge = (reputation) => {
  if (reputation == null || reputation < 10) return { label: "Newcomer", color: "#9CA3AF", icon: "leaf-outline" };
  if (reputation >= 100) return { label: "Local Legend", color: "#F59E0B", icon: "trophy-outline" };
  if (reputation >= 50) return { label: "Trusted", color: "#10B981", icon: "shield-checkmark-outline" };
  return { label: "Regular", color: "#3B82F6", icon: "star-outline" };
};
